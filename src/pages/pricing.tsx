...
export default function Pricing(): ReactNode {
  const productionConversationReceived = new URLSearchParams(useLocation().search).get('conversation') === 'received';
  ...
  return <Layout title="HAPI MCP pricing" description="Keep your API estate AI-ready without creating another MCP codebase to own.">
    ...
    <main className="pricingPage">
      ...
      <section className="pricingSection" id="production-conversation"><div className="container pricingFormLayout"><div><p className="pricingKicker">Production capacity conversation</p><h2>Tell us what you are keeping AI-ready.</h2><p>We use this to recommend the right capacity, Pilot Accelerator, or Enterprise approach.</p></div>{productionConversationReceived ? <div className="pricingFormThanks" role="status"><CheckCircle aria-hidden /><h3>Thank you. Your request is on its way.</h3><p>We will review your production context and follow up with the right next step.</p><a className="button button--lg pricingButtonSecondary" href="/pricing#production-conversation">Send another request</a></div> : <form className="pricingForm" method="post" action={billingEndpoints.salesLead}><div className="pricingFormGrid"><label>Name<input name="firstname" required autoComplete="name" /></label><label>Work email<input name="email" type="email" required autoComplete="email" /></label></div><div className="pricingFormGrid"><label>Company<input name="company" autoComplete="organization" /></label><label>What are you considering?<select name="plan_interest" required defaultValue=""><option value="" disabled>Select one</option><option>Starter</option><option>Pro</option><option>Pilot Accelerator (Thin-Slice)</option><option>Enterprise API estate</option><option>Zero-Shadow Pilot</option></select></label></div><div className="pricingFormGrid"><label>Production API estate<select name="api_estate" required defaultValue=""><option value="" disabled>Select range</option><option>One API</option><option>2–10 APIs</option><option>11–100 APIs</option><option>More than 100 APIs</option></select></label><label>Deployment preference<select name="deployment" required defaultValue=""><option value="" disabled>Select one</option><option>Cloud</option><option>Private cloud</option><option>On-premises or air-gapped</option><option>Not sure yet</option></select></label></div><label>What are you trying to enable?<textarea name="message" rows={4} placeholder="API estate, capacity, security, or migration context" /></label><input type="submit" className="button button--primary button--lg" value="Start the conversation" /></form>}</div></section>
